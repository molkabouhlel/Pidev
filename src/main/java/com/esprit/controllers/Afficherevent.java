...
//recherche
        ...
        recherche.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.BACK_SPACE && recherche.getText().isEmpty()) {
                ObservableList<Evenement> observableList2 = FXCollections.observableList(EV);
                viewev.setItems(observableList2);
            }
        });
    ...
    private void handleSearch(String searchText) {
        ...
        ObservableList<Evenement> filteredList = viewev.getItems().filtered(evenement ->
                evenement.getNom_ev().toLowerCase().contains(searchText.toLowerCase()) ||
                        evenement.getDescription_ev().toLowerCase().startsWith(searchText.toLowerCase()));
